"""
NOTE: Taken from
https://github.com/HRMES-MOPGA/L96HistoryMatching/blob/v0.2.0/GenerateData_Python/L96.py,
code corresponding to original paper by Lguensat et al. (https://arxiv.org/pdf/2208.06243.pdf)
...
class L96OneLevel(object):
    def __init__(
        self, K=36, J=10, h=1, F=10, c=10, b=10, dt=0.001, X_init=None, noprog=False
    ):
    ...
    def _rhs(self, X):
        ...
        dXdt = -np.roll(X, -1) * (np.roll(X, -2) - np.roll(X, 1)) - X + self.F
        return dXdt
    ...
    def history(self):
        ...
        return xr.Dataset(
            {"X": da},
            coords={
                "time": np.arange(len(self._history_X)) * self.dt,
                "x": np.arange(self.K),
            },
        )
...
class L96TwoLevel(object):
    def __init__(
        ...
        # Model parameters
        self.K, self.J, self.h, self.F, self.c, self.b, self.dt = K, J, h, F, c, b, dt
        self.noprog, self.noYhist, self.integration_type = (
            noprog,
            noYhist,
            integration_type,
        )
        ...
        if self.parameterization is not None:
            self.integration_type = "parameterization"
    ...
    def _rhs_X_dt(self, X, Y=None, B=None):
        ...
        if Y is None:
            ...
            dXdt = (
                ...
                - X
                + self.F
                - self.h * self.c * Y.reshape(self.K, self.J).mean(1)
            )
        return self.dt * dXdt
    ...
    def _rhs_Y_dt(self, X, Y):
        ...
        dYdt = (
            ...
            - Y
            + self.h / self.J * np.repeat(X, self.J)
        ) * self.c
        return self.dt * dYdt
    ...
    def step(self, add_B=True, B=None):
        ...
        if self.parameterization is None:
            B = (
                -self.h * self.c * self.Y.reshape(self.K, self.J).mean(1)
                if B is None
                else B
            )
    ...
    def history(self):
        ...
        dic["Y_mean"] = xr.DataArray(
            self._history_Y_mean, dims=["time", "x"], name="Y_mean"
        )
        dic["Y2_mean"] = xr.DataArray(
            self._history_Y2_mean, dims=["time", "x"], name="Y2_mean"
        )
        if not self.noYhist:
            dic["X_repeat"] = xr.DataArray(
                np.repeat(self._history_X, self.J, 1),
                dims=["time", "y"],
                name="X_repeat",
            )
        ...
        return xr.Dataset(
            dic,
            coords={
                "time": np.arange(len(self._history_X)) * self.save_dt,
                "x": np.arange(self.K),
        ...
        if computeY:
            ...
            for u in range(self.J):
                for v in range(u, self.J, 1):
                    Newarray = np.concatenate(
                        [Newarray, (h.Y[:, u] * h.Y[:, v]).values[:, None]], axis=1
                    )
...
class L96TwoLevelOriginal(object):
    ...
    def history(self):
        ...
        dic["Y_sum"] = xr.DataArray(
            self._history_Y_sum, dims=["time", "x"], name="Y_sum"
        )
        dic["Y2_mean"] = xr.DataArray(
            self._history_Y2_mean, dims=["time", "x"], name="Y2_mean"
        )
        ...
        if computeY:
            ...
            for k in range(self.K):
                ...
                for u in range(self.J):
                    for v in range(u, self.J, 1):
                        Newarray = np.concatenate(
                            [Newarray, (h_Y[:, u] * h_Y[:, v])[:, None]], axis=1
                        )
...
class AMIP(object):
    ...
    def _rhs_Y_dt(self, X, Y):
        ...
        dYdt = (
            -self.b * self.c * np.roll(Y, -1) * (np.roll(Y, -2) - np.roll(Y, 1))
            - self.c * Y
            + (self.h * self.c / self.b) * np.repeat(X, self.J)
        )
        return self.dt * dYdt
...
class OMIP(object):
    def __init__(
        ...
        self._history_X = [self.X.copy()]
        self._history_Y_mean = [
            self.Ysumdata[self.step_count, :].copy().values / self.J
        ]
    ...
    def step(self, add_B=True, B=None):
        """Integrate one time step"""
        B = (
            -self.G * self.Ysumdata[self.step_count, :].copy().values
            if B is None
            else B
        )
